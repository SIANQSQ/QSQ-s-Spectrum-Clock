...
u8 DS1302_read_rig(u8 address)//从指定地址读取一字节数据
{
	...
	for(count=0;count<8;count++)
		{
		eDelay_us(3);//使电平持续一段时间
		return_data>>=1;
		SCLK_H;eDelay_us(5);//使高电平持续一段时间
		SCLK_L;eDelay_us(30);//延时14us后再去读取电压，更加准确
...
void DS1302_Init(void)
{
	...
	DS1302_wirte_rig(0x82,0x43);//minutes30分
	DS1302_wirte_rig(0x84,0x12);//hours20时
	DS1302_wirte_rig(0x86,0x06);//date3日
	DS1302_wirte_rig(0x88,0x02);//months10月
